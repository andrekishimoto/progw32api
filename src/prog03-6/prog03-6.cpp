...
//  wcl.lpszMenuName = MAKEINTRESOURCE(IDM_MEUMENU); // Carrega menu do recurso (método 1)
  ...
  if(RegisterClassEx(&wcl))
  {
    ...
    if(hWnd)
    {
	  ...
	  // Deixa item 1 do menu 1 indisponível
	  ...
	  // Checa o item 2 do menu 2 como rádio
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg)
  {
	...
	case WM_RBUTTONUP:
	{
	  // Obtém coordenadas (x, y) do cursor do mouse
	  POINT ptMouse = { LOWORD(lParam), HIWORD(lParam) };

	  // Converte coordenadas do cursor do mouse para coordenadas da tela
	  // Necessário para TrackPopupMenu()
	  ClientToScreen(hWnd, &ptMouse);

	  // Obtém o identificador do menu do programa
	  HMENU hMenuDoProg = GetMenu(hWnd);

	  // Obtém o identificador do sub-menu 1
	  HMENU hMenu1 = GetSubMenu(hMenuDoProg, 0);

	  // Obtém o identificador do sub-menu 2
	  ...
	  // Mostra o menu pop-up na posição do cursor do mouse
