...
//
// prog05-7.cpp - Inversão de cores e preenchimento de áreas
//--------------------------------------------------------------------------
...
#define WINDOW_CLASS		"prog05-7"
// Título da janela 
#define WINDOW_TITLE		"Prog 05-7"
// Largura da janela 
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg)
  {
    ...
    case WM_PAINT: // Janela (ou parte dela) precisa ser atualizada
    {
	  ...
	  // Define os pontos do polígono (estrela)
	  POINT ptPoly[5];
	  ptPoly[0].x = 45 ; ptPoly[0].y = 50;
	  ptPoly[1].x = 155; ptPoly[1].y = 50;
	  ptPoly[2].x = 60 ; ptPoly[2].y = 115;
	  ptPoly[3].x = 100; ptPoly[3].y = 15;
	  ptPoly[4].x = 140; ptPoly[4].y = 115;

	  // Define modo de preenchimento completo
	  SetPolyFillMode(hDC, WINDING);

	  // Desenha o polígonos
	  Polygon(hDC, ptPoly, 5);

	  // Desenha retângulo
	  Rectangle(hDC, 200, 10, 300, 200);

	  // Inverte o retângulo
	  RECT rcRect = { 200, 10, 300, 200 };
	  InvertRect(hDC, &rcRect);

	  // Seleciona hBrush2 no DC
	  SelectObject(hDC, hBrush2);

	  // Preenche área que for da cor branca, começando em (100, 100);
	  ExtFloodFill(hDC, 100, 100, RGB(255, 255, 255), FLOODFILLSURFACE);
