...
//
// prog05-6.cpp - Desenhando polígonos
//--------------------------------------------------------------------------
...
#define WINDOW_CLASS		"prog05-6"
// Título da janela 
#define WINDOW_TITLE		"Prog 05-6"
// Largura da janela 
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg)
  {
    ...
    case WM_PAINT: // Janela (ou parte dela) precisa ser atualizada
    {
	  ...
	  POINT ptPoly[5];
	  ptPoly[0].x = 35 ; ptPoly[0].y = 50;
	  ptPoly[1].x = 145; ptPoly[1].y = 50;
	  ptPoly[2].x = 50 ; ptPoly[2].y = 115;
	  ptPoly[3].x = 90 ; ptPoly[3].y = 15;
	  ptPoly[4].x = 130; ptPoly[4].y = 115;

	  // Desenha o polígono
	  Polygon(hDC, ptPoly, 5);

	  // Modifica posição x do polígono
	  for(int i = 0; i < 5; i++)
		  ptPoly[i].x += 130;
	  ...
	  // Desenha o polígono na nova posição
	  Polygon(hDC, ptPoly, 5);
